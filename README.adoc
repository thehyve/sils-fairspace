...
Another way to update metadata is by uploading a comma-separated values (CSV) file with metadata.
...
* _DirectoryName_: the of a directory in the current folder.
...
If Jupyter Hub extension is added, after opening Jupyter users are automatically authenticated and can directly connect to the
...
This project was realized thanks to and funded by Swammerdam Institute for Life Sciences (SILS).
